@extends('layouts.app')
@section('title', 'Edit Kategori - PerpusTech')
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-soft-periwinkle">Edit Kategori</h1>
    <p class="text-gray-500 mt-1">Ubah nama klasifikasi kategori ini.</p>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-periwinkle/30 p-8 max-w-xl">
    <form action="{{ route('kategori.update', $kategori->kategori_id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
